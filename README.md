...
A disadvantage of refactoring is that an indiviudal may break a code that previously worked or introduce bugs. Because of this, during this project a copy of the original code was kept in order to compare logic and resulting outputs, and the new code was saved frequently in order to return to previous versions if a mistake was made. For example, on one occasion the refactored code ran to completion, but the output was completely different than the ouput from the original code. This may have gone unnoticed and not been as easy to fix had the original code not been kept to use for comparison.   
